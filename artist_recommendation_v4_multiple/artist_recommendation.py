...
#charger les donnees data.json dans la collection job_IA de la BDD jo_search_db
#mongoimport --jsonArray -d job_search_db -c job_IA --file data.json 
...
def get_recommendation_LightFM(artist):
    global ratings, artists, ratings_df, ap, artist_rank, user_ids, n_users
    ...
    if artist != '':    
        ...
        for art in artist:
            ...
            if artists[artists['name']==art].empty == False:
                ...
                print('trouve :',artist_col)
            else :
                artist_col = 300 #Black Eyed Peas
                print('pas trouve :',artist_col)
...
def get_top_artists_listened():
    ...
    #artist_names_listened = ap.sort_values("userID", ascending = False)["name"].unique()
...
def most_rec_artists():
    global user_ids, n_users
    artist_names_rec_hist = []
    for i in range(100):
        #userID = user_ids[np.random.randint(n_users)]
...
def index():
    ...
    return render_template('layout.html', result=top_items[0:18])
